...
const ALLOWED_ROOT = new Set(["package.json", "README.md", "LICENSE", "tokens.css", "theme.css"]);
...
for (const required of ["tokens.css", "theme.css", "dist/react/index.js", "dist/svelte/index.js"]) {
  if (!files.some((f) => f.path === required)) problems.push(`missing from tarball: ${required}`);
}

for (const p of problems) console.error(`FAIL pack: ${p}`);
if (problems.length === 0) console.log(`pack: ${files.length} files, all within the allowlist`);
